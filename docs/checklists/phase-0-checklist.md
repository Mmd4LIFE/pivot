# Phase 0 — Foundations · build checklist (closed)

**This phase is complete.** It is kept as the record of what was built, what was
decided and what went wrong, part by part. The working checklist is
[../../CHECKLIST.md](../../CHECKLIST.md), which now covers Phase 1.

Phase 0 ran from 2026-09-19 to 2026-09-26 and produced 31 parts across 15
numbered steps. The exit review is
[../roadmap/phase-0-exit-review.md](../roadmap/phase-0-exit-review.md).

The procedure below is how every session in this phase worked, and how sessions
in the next one still work.
...
4. **Read the linked roadmap doc** for that part before writing code. The feature IDs
   (e.g. `P0-AUTH-001`) map to specs in [docs/roadmap/](../roadmap/).
5. **Build it.** Follow [docs/roadmap/00-principles.md](../roadmap/00-principles.md) —
...
| | |
|---|---|
| **Last completed** | Part 15-e — The container stack, and Phase 0 close-out |
| **Next up** | Phase 1 — see [../../CHECKLIST.md](../../CHECKLIST.md) |
...
justified in [ADR-0001](../architecture/adr/0001-backend-language.md#amendments)).
...
[docs/operations/secrets.md](../operations/secrets.md) is the procedure.
...
[docs/operations/backup-and-restore.md](../operations/backup-and-restore.md) is the
...
[ADR-0009's amendment](../architecture/adr/0009-authorization.md#amendments) for the
...
[docs/design/keyboard-audit.md](../design/keyboard-audit.md) is the map.
...
- Go 1.27.1 via snap (`/snap/bin/go`). **`go.mod` floor is 1.26.0** — raised in Part 3-a
  ...
  [ADR-0001](../architecture/adr/0001-backend-language.md#amendments).
...
> serves a React app. Spec: [docs/roadmap/phase-0-foundations.md](../roadmap/phase-0-foundations.md)
...
- Schema v1 per [data-model.md](../architecture/data-model.md#1-identity--tenancy):
...
session log: [ADR-0003](../architecture/adr/0003-metadata-database.md) sets a 15%
...
[ADR-0009 amendment](../architecture/adr/0009-authorization.md#amendments).
...
- Tailwind 4 with **design tokens as CSS custom properties** — not compiled class names;
  [white-label embedding depends on this](../architecture/adr/0002-frontend-stack.md)
...
[ADR-0002](../architecture/adr/0002-frontend-stack.md#amendments) chose anyway. **Part
...
- [~] Every component is keyboard navigable with a visible focus indicator —
      **partly.** 21 tests prove where focus *goes*; `TestNothingRemovesTheFocusIndicator`
      ...
      [docs/design/keyboard-audit.md](../design/keyboard-audit.md) and **has not been
...
- [x] **Zero axe violations in a real browser**, six pages plus the login page, the
      ...
      for
- [~] The manual items in [docs/design/keyboard-audit.md](../design/keyboard-audit.md)
...
[00-principles.md](../roadmap/00-principles.md#5-quality-gates-in-ci) exists, runs, and
...
[00-principles.md](../roadmap/00-principles.md#5-quality-gates-in-ci)
...
reasoning in [ADR-0001](../architecture/adr/0001-backend-language.md#amendments),
...
**Found on the way:**
- **The coverage gate failed on CI at 75.8% where it read 84.4% locally**, and the cause
  is worth remembering: the race test covered the retry paths only when the goroutines
  actually interleaved, which a two-core runner never did. Timing-dependent coverage is
  not coverage. The wait path now has a **deterministic** test — an empty key file plus a
  sibling that finishes writing it 80ms later — and the package is at 90.1%. CI's older
  toolchain also counts fewer statements than Go 1.27 does (182 against 212), so the two
  numbers never match exactly; the margin has to absorb that
...
### - [x] Part 15-e — The container stack, and Phase 0 close-out ✅ 2026-09-26
...
**Done when:**
- [x] **`docker compose up` brings up the stack and serves a working Pivot**, walked:
      setup → 201, login → 200, the application → 200, and the data survives a restart
- [x] **Both install paths verified.** The binary was timed from an empty directory —
      **226 ms to serving, 399 ms to having an administrator** — and the container path was
      walked end to end against PostgreSQL
- [x] **Phase 0 exit criteria all walked**, one at a time, with what was actually run
      recorded in [the exit review](../roadmap/phase-0-exit-review.md). Six were met. The
      seventh was not, and is now
- [x] Phase 1 parts 16–27 expanded, in [../../CHECKLIST.md](../../CHECKLIST.md)

**Found on the way:**
- **The published image tag has no `v`.** The release workflow does
  `version="${GITHUB_REF_NAME#v}"`, so the tag is `0.0.1-alpha`. My compose file said
  `v0.0.1-alpha` and would have failed to pull on the first `docker compose up` anybody
  tried — the exact failure an unverified deployment file carries
- **The pinned release predates the setup wizard.** `0.0.1-alpha` was cut after Part 13;
  parts 14 and 15 came after it. The stack came up healthy on that tag and `/setup` was a
  401, because the route did not exist yet. Closing the phase means cutting the release
  that contains it
- **"OIDC login works end to end against Keycloak" was not true.** The test that existed
  covered discovery and the shape of the authorization request — and skipped unless an
  environment variable was set, so it had never run at all. The full round trip works: I
  walked it by hand against Keycloak 26 first, then wrote `TestKeycloakLoginRoundTrip`,
  which provisions its own client and user through the admin API and drives Keycloak's own
  login form through to a Pivot session
- **Valkey and MinIO are not in the compose file**, though the plan named them. Nothing
  connects to them until Phase 9 and Phase 5. A compose file that starts services the
  product never talks to teaches operators they are required
...
> Spec: [docs/roadmap/phase-1-connect-and-query.md](../roadmap/phase-1-connect-and-query.md)
...
- [ ] **Part 23** — Three-tier cache with **RLS-ready cache keys** *(see [ADR-0006](../architecture/adr/0006-caching-strategy.md))*
...
| Phase | Release | Spec |
|---|---|---|
| 2 — Visualize & Dashboard | v0.3 | [phase-2](../roadmap/phase-2-visualization-dashboards.md) |
| 3 — Semantic Layer | v0.5 | [phase-3](../roadmap/phase-3-semantic-layer.md) |
| 4 — Collaborate & Govern | v0.7 | [phase-4](../roadmap/phase-4-collaboration-governance.md) |
| 5 — Alert & Distribute | v0.9 | [phase-5](../roadmap/phase-5-alerting-distribution.md) |
| 6 — Flows | **v1.0 GA** | [phase-6](../roadmap/phase-6-flows.md) |
| 7 — Pivot AI | v1.5 | [phase-7](../roadmap/phase-7-ai.md) |
| 8 — Embed & Extend | v1.8 | [phase-8](../roadmap/phase-8-embedded-extensibility.md) |
| 9 — Enterprise & Scale | v2.0 | [phase-9](../roadmap/phase-9-enterprise-scale.md) |
| 10 — Ecosystem | continuous | [phase-10](../roadmap/phase-10-ecosystem.md) |
...
|---|---|---|---|
| 2026-09-26 | 15-e | `deploy/docker-compose.yml`, `.env.example`, `docs/operations/installing.md`, `TestKeycloakLoginRoundTrip`, and [the Phase 0 exit review](../roadmap/phase-0-exit-review.md) | **Phase 0 closes.** The container path was walked end to end — setup 201, login 200, the application 200, data surviving a restart — and the binary path was timed from an empty directory at **226 ms to serving and 399 ms to an administrator**, so the thirty-second promise is bounded by typing speed rather than software. **Three things the walk found that the plan did not.** The published image tag has no `v` — the release workflow strips it — so my compose file's `v0.0.1-alpha` would have failed to pull on the first `docker compose up` anybody tried. The pinned release **predates the setup wizard**: the stack came up healthy on `0.0.1-alpha` and `/setup` returned 401, because the route did not exist when that tag was cut. And **the Keycloak exit criterion was not met**: the test that existed covered discovery and the shape of an authorization request, and skipped unless an environment variable was set, so it had never run. I walked the real login by hand against Keycloak 26 — client and user provisioned through the admin API, Keycloak's own login form, the callback, a Pivot session for an auto-provisioned Grace Hopper — and then wrote `TestKeycloakLoginRoundTrip` so nobody has to do it by hand again. **Valkey and MinIO are deliberately absent** from the compose file the plan said would contain them: nothing connects to them until Phases 9 and 5, and a compose file that starts services the product never talks to teaches operators they are required. Six exit criteria were met as written, the seventh is met now, and three small truths are carried into Phase 1 rather than quietly dropped. |
| 2026-09-23 | 15-d | `internal/secrets`, an encrypting decorator over the `Querier`, `pivot secrets status\|rewrap\|generate-key`, a `doctor` check, and the rotation procedure | **The client secret that has been plaintext since Part 7 is an envelope now** — a per-value data key wrapped by a master key held outside the database — and the test checks that with SQL against the file rather than through the code that wrote it, because asking the application whether it encrypted something is asking the guard whether the door is locked. **Envelope rather than encrypting with the master key directly**, because rotation then rewraps data keys instead of re-encrypting data: the same work for five secrets or five thousand. **The ciphertext is bound to its column** through AEAD's additional data, so a value lifted out of one cannot be pasted into another and decrypted there. **A key's ID is a fingerprint of its own material**, so two instances given the same key agree on its identity without being told, and nobody can rotate the label while leaving the key. **Encryption lives in a decorator over the `Querier`, like tracing**: a repository method that forgets to encrypt is invisible, while a decorator that does not mention a column is a gap somebody can see. **The default cipher cannot encrypt** — an instance with no key refuses to write a secret rather than quietly writing it in the clear, which is the exact failure this part exists to remove, and it broke two tests that were then given a real keyring instead of a no-op. **Rotation was walked, not described**: new key primary with the old retained, rewrap, drop the old key, everything still readable — and the document says in those words that doing the last step first is unrecoverable. Legacy plaintext reads through unchanged, because an upgrade that breaks every SSO login is not an upgrade. **What this does not protect against is written down too**: Pivot decrypts on demand, so root on the host can have the plaintext; that needs a KMS, which the envelope's key identifier leaves room for. **And a bug in my own key handling, found by repetition**: two instances starting at once could both refuse to start, because `O_EXCL` makes the key file appear before its contents do — the loser read an empty file and declared the winner's key unusable, with a message telling the operator not to delete it. There were two such windows, not one; the second was still failing after the first was fixed, and only twenty repetitions under `-race` showed it. **Then CI's coverage gate refused it at 75.8% where it read 84.4% locally**, and the reason is the lesson: the race test covered the retry paths only when the goroutines actually interleaved, and a two-core runner never let them. Timing-dependent coverage is not coverage. The wait now has a deterministic test — an empty key file and a sibling that finishes writing it 80ms later — and the package is at 90.1%. Worth carrying forward: CI's older toolchain counts fewer statements than Go 1.27 does (182 against 212), so the two percentages never match and the margin has to absorb the difference. |
