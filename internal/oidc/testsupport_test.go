...
func openSQLite(t *testing.T) *store.DB {
	...
	db, err := store.Open(context.Background(),
		testDBConfig(filepath.Join(t.TempDir(), "oidc-test.db")), discardLogger())
	if err != nil {
...
// fixture is a migrated database with one organization and a provisioner.
type fixture struct {
...
// newFixture migrates a database and creates one organization, returning a
// context already scoped to it.
func newFixture(t *testing.T, db *store.DB) *fixture {
	...
	return &fixture{
		repos:       repos,
		provisioner: oidc.NewProvisioner(repos, discardLogger()),
		ctx:         ctx,
		org:         org,
	}
}
